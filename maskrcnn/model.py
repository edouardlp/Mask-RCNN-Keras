...
class MaskRCNNModel():
    ...
    def predict(self, image_infos, images):
    ...
    def predict(self, dataset):
        estimator = self._get_estimator()
        #TODO: transform results
        return estimator.predict(dataset.make_eval_input_fn())
    ...
    def get_trained_keras_models(self):
        ...
        fpn_mask_model, \
        anchors = self._get_keras_models()
    ...
    def _get_keras_models(self):
        if self._models is None:
            mask_rcnn_model, \
            fpn_classifier_model, \
            fpn_mask_model, \
            anchors = self._build_keras_models()
            self._models = [mask_rcnn_model, fpn_classifier_model, fpn_mask_model]
            self._anchors = anchors

        mask_rcnn_model, fpn_classifier_model, fpn_mask_model = self._models
        return mask_rcnn_model, fpn_classifier_model, fpn_mask_model, self._anchors
    ...
    def _build_estimator(self):
        ...
        mask_rcnn_model, _, _, _ = self._get_keras_models()

        #TODO: load the initial keras weights
        ...
        return keras.estimator.model_to_estimator(mask_rcnn_model)
